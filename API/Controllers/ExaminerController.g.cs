...
{
    /// <summary>
    /// Controller responsible for managing examiner related operations.
    /// </summary>
    /// <remarks>
    ...
    public class ExaminerController : ControllerBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the ExaminerController class with the specified context.
        /// </summary>
        /// <param name="iexaminerservice">The iexaminerservice to be used by the controller.</param>
        public ExaminerController(IExaminerService iexaminerservice)
        {
            _examinerService = iexaminerservice;
        }
        ...
        {
            var id = _examinerService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _examinerService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _examinerService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _examinerService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Examiner updatedEntity)
        {
            ...
            var status = _examinerService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Examiner> updatedEntity)
        {
            ...
            var status = _examinerService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}