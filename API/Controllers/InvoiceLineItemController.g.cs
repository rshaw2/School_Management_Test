...
{
    /// <summary>
    /// Controller responsible for managing invoicelineitem related operations.
    /// </summary>
    /// <remarks>
    ...
    public class InvoiceLineItemController : ControllerBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the InvoiceLineItemController class with the specified context.
        /// </summary>
        /// <param name="iinvoicelineitemservice">The iinvoicelineitemservice to be used by the controller.</param>
        public InvoiceLineItemController(IInvoiceLineItemService iinvoicelineitemservice)
        {
            _invoiceLineItemService = iinvoicelineitemservice;
        }
        ...
        {
            var id = _invoiceLineItemService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _invoiceLineItemService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _invoiceLineItemService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _invoiceLineItemService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] InvoiceLineItem updatedEntity)
        {
            ...
            var status = _invoiceLineItemService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<InvoiceLineItem> updatedEntity)
        {
            ...
            var status = _invoiceLineItemService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}