...
namespace SchoolManagementTest.Controllers
{
    ...
    public class ContactController : ControllerBase
    {
        ...
        {
            var id = _contactService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _contactService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] Contact updatedEntity)
        {
            ...
            var status = _contactService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Contact> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = _contactService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}