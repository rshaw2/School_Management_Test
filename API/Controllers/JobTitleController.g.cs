...
{
    /// <summary>
    /// Controller responsible for managing jobtitle related operations.
    /// </summary>
    /// <remarks>
    ...
    public class JobTitleController : ControllerBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the JobTitleController class with the specified context.
        /// </summary>
        /// <param name="ijobtitleservice">The ijobtitleservice to be used by the controller.</param>
        public JobTitleController(IJobTitleService ijobtitleservice)
        {
            _jobTitleService = ijobtitleservice;
        }
        ...
        {
            var id = _jobTitleService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _jobTitleService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _jobTitleService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _jobTitleService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JobTitle updatedEntity)
        {
            ...
            var status = _jobTitleService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<JobTitle> updatedEntity)
        {
            ...
            var status = _jobTitleService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}